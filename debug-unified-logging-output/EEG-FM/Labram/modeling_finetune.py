...
@register_model
def labram_base_d6_patch200_200(pretrained=False, **kwargs):
    """Half-depth variant of the base model (6 instead of 12 transformer blocks).

    Keeps embed_dim=200 so the first 6 blocks can be warm-started directly from
    the pretrained labram_base_patch200_200 checkpoint without any projection.
    Estimated size: ~3.2M parameters (55% of the full base model).
    """
    model = NeuralTransformer(
        patch_size=200, embed_dim=200, depth=6, num_heads=10, mlp_ratio=4,
