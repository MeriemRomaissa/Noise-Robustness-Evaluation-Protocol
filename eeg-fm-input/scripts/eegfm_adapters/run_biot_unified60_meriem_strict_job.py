#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run one BIOT unified60 Meriem-strict strategy job and annotate metrics."""
...
BIOT_REPO = "/nicoletye/workspace/Noise Robustness Evaluation Protocol/EEG-FM/Biot"
BIOT_VENV = "/nicoletye/venvs/biot_env"
BIOT_PYTHON = f"{BIOT_VENV}/bin/python"
BIOT_CHECKPOINT = (
    "/nicoletye/workspace/Noise Robustness Evaluation Protocol/EEG-FM/Biot/"
    "pretrained-models/EEG-PREST-16-channels.ckpt"
)
BIOT_NATIVE_CONFIG = {
    "class": "BIOTClassifier",
    "n_classes": 2,
    "n_channels": 16,
    "n_fft": 200,
    "hop_length": 200,
...
STRATEGY_CLASSIFICATION = {
    "full_finetune": "STRICT_MATCH",
    "linear_probe": "FUNCTIONAL_MATCH_EXACT_TRAINABLE_SET",
    "lora": "FUNCTIONAL_MATCH_WITH_ARCHITECTURE_DIFFERENCE",
}
...
def bool_head_trainable(trainable_names: list[str]) -> bool:
    return any(name.startswith("classifier.") or name.startswith("classifier.clshead.") for name in trainable_names)
...
def annotate_metrics(metrics_path: Path, args: argparse.Namespace, returncode: int) -> None:
    ...
    backbone_frozen = args.strategy in {"linear_probe", "lora"} and all(
        name.startswith("classifier.") or "lora_" in name.lower() for name in trainable_names
    )
    ...
    metrics.update(
        {
            ...
            "checkpoint_present": bool(checkpoint_report.get("checkpoint_exists", Path(BIOT_CHECKPOINT).exists())),
            "checkpoint_loaded": bool(checkpoint_report.get("checkpoint_loaded", False)),
            "checkpoint_path": checkpoint_report.get("checkpoint_path", BIOT_CHECKPOINT),
            ...
            "native_architecture_used": BIOT_NATIVE_CONFIG,
            ...
            "lora_comparability": (
                "FUNCTIONAL_MATCH_WITH_ARCHITECTURE_DIFFERENCE" if args.strategy == "lora" else ""
            ),
            ...
            "biot_checkpoint_reference": BIOT_CHECKPOINT,
            "biot_native_config_reference": BIOT_NATIVE_CONFIG,
        }
    )
    if args.strategy == "lora":
        ...
        metrics["notes"] = (
            ...
            + " BIOT LoRA uses BIOT-native architecture-compatible attention/projection/MLP-like targets; "
...
def main() -> int:
    ...
    config = {
        ...
        "strategy": args.strategy,
        "seed": args.seed,
        "checkpoint_path": BIOT_CHECKPOINT,
        "native_config": BIOT_NATIVE_CONFIG,
