#!/usr/bin/env python3
"""CodeBrain Option 1 TUAB preprocessing wrapper.

Project-local wrapper for the matched-subset comparison. CodeBrain does not
ship a TUAB raw EDF maker in the inspected tree, so this preserves the repo's
TUAB PKL input contract while keeping the approved Option 1 train/val/test
split and never scanning the full TUAB root.
"""
...
DEFAULT_OUTPUT_ROOT = ROOT / "data" / "codebrain_option1_original_processed"
REPORT_JSON = ROOT / "reports" / "codebrain_option1_preprocessing_wrapper_summary.json"
REPORT_MD = ROOT / "reports" / "codebrain_option1_preprocessing_wrapper_report.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="CodeBrain Option 1 TUAB preprocessing wrapper.")
...
def write_reports(summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# CodeBrain Option 1 Preprocessing Wrapper Report",
        "",
        ...
        "",
        "## CodeBrain TUAB-Compatible Transform",
        "",
        "- No CodeBrain TUAB raw EDF maker was found in the inspected repo tree.",
        "- The repo TUAB loader expects PKL files under `train`, `val`, and `test`.",
        "- Each PKL contains `X` and `y`; the loader reshapes `X` to `(16, 10, 200)` and divides by 100.",
        "- Resample to 200 Hz.",
