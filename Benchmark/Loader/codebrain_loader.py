"""Prepare original or unified60 TUAB windows for CodeBrain."""
...
def get_loader_settings() -> dict:
    """Describe the preprocessing for CodeBrain."""
    return {
        "model": "CodeBrain",
        "channels": 16,
        "montage": "bipolar",
...
def build_unified60_loader(config: dict, split: str):
    """Load unified60 data and apply the complete CodeBrain input recipe."""
...
def validate_loader_batch(batch) -> bool:
    """Confirm that a CodeBrain batch is [B,16,10,200]."""
    eeg = batch[0] if isinstance(batch, (tuple, list)) else batch
    shape = shape_of(eeg)
    if len(shape) == 4 and shape[1:] == (16, 10, 200):
        return True
    raise ValueError(f"CodeBrain batch must be [B,16,10,200], got {list(shape)}")
