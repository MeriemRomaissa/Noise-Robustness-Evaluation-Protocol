"""Prepare bipolar TUAB windows for CodeBrain."""
...
def build_unified60_loader(config, split):
    """Read referential unified60 windows and prepare CodeBrain input."""
    return build_h5_loader(config, split, transform_h5_window)


def build_original_loader(config, split):
    """Read original CodeBrain windows that are already bipolar."""
...
def transform_pkl_window(window, channel_names=None, config=None):
    """Scale and patch original CodeBrain data without reconstructing its montage."""
...
def validate_loader_batch(batch):
    """Confirm CodeBrain receives ``[B,16,10,200]``."""
    expected_shape = (N_CHANNELS, N_PATCHES, PATCH_SAMPLES)
    return check_batch_shape(batch, [expected_shape], "CodeBrain")
