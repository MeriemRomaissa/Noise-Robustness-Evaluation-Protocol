#!/usr/bin/env python3
"""Connector for EEG-FM/BIOT/run_binary_supervised.py.

BIOT has one original fine-tuning path. The Benchmark connector loads YAML as
defaults through the author script and only appends explicit CLI overrides.
"""
...
BIOT_SCRIPT = REPO_ROOT / "EEG-FM" / "Biot" / "run_binary_supervised.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Additional native BIOT args after '--', for example: -- --model BIOT",
    )
...
def main() -> int:
    ...
    return subprocess.run(cmd, cwd=BIOT_SCRIPT.parent, env=env).returncode
