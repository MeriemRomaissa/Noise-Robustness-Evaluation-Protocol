#!/usr/bin/env python3
"""Connector for EEG-FM/CSBrain/finetune_main.py.

CSBrain has one original fine-tuning path. The Benchmark connector loads YAML
...
CSBRAIN_SCRIPT = REPO_ROOT / "EEG-FM" / "CSBrain" / "finetune_main.py"
...
def build_cmd(args: argparse.Namespace) -> list[str]:
    ...
    append_if_set(cmd, "--scale_mode", args.scale_mode)
    ...
    append_if_set(cmd, "--model_dir", output_dir)
    #newly added codes
...
def resolve_repo_path(value: str | Path | None) -> Path | None:
    """Resolve output paths before changing to the CSBrain directory."""
    if value is None:
        return None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CSBrain through the Benchmark YAML connector.")
    ...
    parser.add_argument("--scale_mode", default=None, choices=["mul10000", "mul1000", "div100"], help="Override CSBrain loader scale mode.")
    ...
    parser.add_argument(
        ...
        help="Additional native CSBrain args after '--', for example: -- --model CSBrain",
    )
...
def main() -> int:
    ...
    return subprocess.run(cmd, cwd=CSBRAIN_SCRIPT.parent, env=env).returncode
