...
CSBRAIN_ROOT = REPO_ROOT / "EEG-FM" / "CSBrain"
BENCHMARK_LOADER = REPO_ROOT / "Benchmark" / "DataLoader"
DEFAULT_CONFIG = REPO_ROOT / "Benchmark" / "Config" / "csbrain.yaml"
...
from lora_csbrain import apply_lora_strategy as apply_model_lora_strategy
from loader_csbrain import transform_h5_window
...
class NmtPickleDataset(Dataset):
    """Read NMT samples and adapt them to CSBrain's TUAB input shape."""
    def __init__(self, root: Path, transform_config: dict, max_samples: int | None = None):
        self.root = Path(root)
    ...
    def __getitem__(self, idx):
        ...
        x = transform_h5_window(sample["X"], TUAB_CH, self.transform_config)
...
def write_nmt_outputs(output_dir: Path, results: dict):
    ...
    records = [
        benchmark_log_record("CSBrain Full FT", results["full_ft"]["nmt_ood"],
                             results["full_ft"]["checkpoint"], "NMT_OOD"),
    ]
    if "freeze_backbone" in results:
        records.append(
            benchmark_log_record("CSBrain Freeze Backbone", results["freeze_backbone"]["nmt_ood"],
                                 results["freeze_backbone"]["checkpoint"], "NMT_OOD")
        )
    if "lora" in results:
        records.append(
            benchmark_log_record("CSBrain LoRA", results["lora"]["nmt_ood"],
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run CSBrain NMT OOD inference.")
    ...
    parser.add_argument("--scale_mode", choices=["mul10000", "mul1000", "div100"], default=None)
...
def main():
    ...
    transform_config = {"scale_mode": args.scale_mode or data.get("scale_mode", "div100")}
    ...
    if args.dry_run:
        ...
        print(f"scale_mode={transform_config['scale_mode']}")
    ...
    dataset = NmtPickleDataset(nmt_dir, transform_config, max_samples=max_samples)
    ...
    model = load_checkpoint(build_model(), checkpoints["full_finetune"])
    ...
    results = {
        "description": "CSBrain full fine-tuning vs EEGNet NMT OOD inference from Benchmark/Inference.",
        "nmt_ood": metrics,
        ...
        "scale_mode": transform_config["scale_mode"],
    }
    if checkpoints["freeze_backbone"]:
        model_freeze = load_strategy_checkpoint(build_model(), checkpoints["freeze_backbone"], "freeze_backbone", output_dir)
    ...
    if checkpoints["lora"]:
        model_lora = load_strategy_checkpoint(build_model(), checkpoints["lora"], "lora", output_dir)
