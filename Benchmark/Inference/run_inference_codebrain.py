...
def write_nmt_outputs(output_dir: Path, results: dict):
    ...
    if "freeze_backbone" in results:
        records.append(
            benchmark_log_record("CodeBrain Freeze Backbone", results["freeze_backbone"]["nmt_ood"],
                                 results["freeze_backbone"]["checkpoint"], "NMT_OOD")
        )
    if "lora" in results:
        records.append(
            benchmark_log_record("CodeBrain LoRA", results["lora"]["nmt_ood"],
