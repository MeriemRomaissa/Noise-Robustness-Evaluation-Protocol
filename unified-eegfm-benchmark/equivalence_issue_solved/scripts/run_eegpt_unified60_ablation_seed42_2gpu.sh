#!/usr/bin/env bash
set -euo pipefail

cd /nicoletye/workspace/unified_tuab

export PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION="${PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION:-python}"
export PYTHONUNBUFFERED="${PYTHONUNBUFFERED:-1}"
export OMP_NUM_THREADS="${OMP_NUM_THREADS:-4}"
export MKL_NUM_THREADS="${MKL_NUM_THREADS:-4}"

GPU_LIST="${GPU_LIST:-0,1}"
MAX_PARALLEL="${MAX_PARALLEL:-2}"
DRY_RUN="${DRY_RUN:-0}"

IFS=',' read -r -a GPUS <<< "$GPU_LIST"
if [[ "${#GPUS[@]}" -eq 0 ]]; then
  echo "ERROR: GPU_LIST is empty" >&2
  exit 2
fi
if [[ "$MAX_PARALLEL" -gt "${#GPUS[@]}" ]]; then
  MAX_PARALLEL="${#GPUS[@]}"
fi

INDEX_NPZ="reports/all6_unified60_subset_epoch15_5seed_1gpu_dev_v1/all6_fixed_subset_seed42_index.npz"
H5_PATH="data/canonical_tuab_full.h5"
OUTPUT_ROOT="outputs/eegpt_unified60_ablation_seed42_2gpu"
REPORT_DIR="reports/all6_original_repo_vs_90job_unified60_matchedsplit_epoch15_seed42_v1"
LAUNCH_LOG="$REPORT_DIR/eegpt_unified60_ablation_seed42_2gpu_launch.log"
SUMMARY_SCRIPT="scripts/eegfm_adapters/summarize_eegpt_unified60_ablation_seed42.py"
PYTHON="/nicoletye/venvs/eegpt_labram/bin/python"
JOB_SCRIPT="scripts/eegfm_adapters/run_eegpt_unified60_meriem_strict_job.py"

mkdir -p "$OUTPUT_ROOT" "$REPORT_DIR"
: > "$LAUNCH_LOG"

log() {
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*" | tee -a "$LAUNCH_LOG"
}

command_text() {
  /nicoletye/venvs/eegnet/bin/python - "$@" <<'PY'
import shlex, sys
print(shlex.join(sys.argv[1:]))
PY
}

wait_for_slot() {
  local rc
  while [[ "$(jobs -pr | wc -l)" -ge "$MAX_PARALLEL" ]]; do
    set +e
    wait -n
    rc=$?
    set -e
    if [[ "$rc" -ne 0 ]]; then
      log "A background ablation job exited with rc=$rc"
    fi
  done
}

run_job() {
  local job_index="$1"
  local name="$2"
  local lr="$3"
  local weight_decay="$4"
  local num_workers="$5"
  local train_drop_last="$6"
  local checkpoint_mapping="$7"
  local epochs=15
  local train_n=8192
  local val_n=2048
  local test_n=2048
  local gpu="${GPUS[$((job_index % ${#GPUS[@]}))]}"
  local out_dir="$OUTPUT_ROOT/$name"
  local terminal_log="$out_dir/terminal.log"
  mkdir -p "$out_dir"

  local cmd=(
    "$PYTHON" "$JOB_SCRIPT"
    --strategy full_finetune
    --seed 42
    --output_dir "$out_dir"
    --epochs "$epochs"
    --batch_size 64
    --lr "$lr"
    --weight_decay "$weight_decay"
    --device cuda:0
    --num_workers "$num_workers"
    --pin_memory
    --index_npz "$INDEX_NPZ"
    --h5 "$H5_PATH"
    --train_n "$train_n"
    --val_n "$val_n"
    --test_n "$test_n"
    --eegpt_checkpoint_mapping "$checkpoint_mapping"
    --train_drop_last "$train_drop_last"
    --lora_rank 2
    --lora_alpha 8
    --lora_target meriem_exact
  )
  local text
  text="$(command_text "${cmd[@]}")"
  log "JOB $name gpu=$gpu output=$out_dir"
  log "COMMAND $text"

  if [[ "$DRY_RUN" == "1" ]]; then
    printf '%s\n' "$text" > "$out_dir/command.txt"
    return 0
  fi

  wait_for_slot
  (
    export CUDA_VISIBLE_DEVICES="$gpu"
    "${cmd[@]}" 2>&1 | tee "$terminal_log"
  ) &
}

log "EEGPT unified60 ablation seed42 launcher"
log "GPU_LIST=$GPU_LIST parsed=${GPUS[*]} MAX_PARALLEL=$MAX_PARALLEL DRY_RUN=$DRY_RUN"
if command -v nvidia-smi >/dev/null 2>&1; then
  nvidia-smi --query-gpu=index,name,memory.used,memory.total,utilization.gpu --format=csv,noheader | tee -a "$LAUNCH_LOG" || true
fi
log "INDEX_NPZ=$INDEX_NPZ"
log "H5_PATH=$H5_PATH"

run_job 0 A0_current_unified_reference 0.0001 0.0005 8 0 encoder_to_target_encoder
run_job 1 A1_original_optimizer_recipe 0.0005 0.05 2 1 encoder_to_target_encoder
run_job 2 A2_checkpoint_mapping_none 0.0001 0.0005 8 0 none
run_job 3 A3_original_recipe_plus_checkpoint_none 0.0005 0.05 2 1 none

if [[ "$DRY_RUN" != "1" ]]; then
  wait
  /nicoletye/venvs/eegnet/bin/python "$SUMMARY_SCRIPT" --output_root "$OUTPUT_ROOT" | tee -a "$LAUNCH_LOG"
fi

log "DONE"
