#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CodeBrain isolated remaining-model Meriem-strict debug job wrapper."""
...
MODEL = 'CodeBrain'
REPO = '/nicoletye/workspace/Noise Robustness Evaluation Protocol/EEG-FM/Codebrain'
    ...
    if metrics_path.exists():
        try:
            ...
            if args.strategy == 'lora':
                metrics['lora_rank'] = 2
                metrics['lora_placement'] = 'MERIEM_STRICT_ARCHITECTURE_NATIVE_EQUIVALENT_PARTIAL'
                metrics['lora_comparability'] = 'PARTIAL_NATIVE_EQUIVALENT'
                metrics['notes'] = (
                    str(metrics.get('notes', '')).strip()
                    + ' CodeBrain LoRA is Meriem-strict partial/native-equivalent; do not describe it as exact LaBraM q/k/v + MLP placement.'
                ).strip()
