#!/usr/bin/env python3
"""BIOT Option 1 TUAB preprocessing wrapper.

Project-local wrapper for the matched-subset comparison. It mirrors BIOT's
original TUAB EDF-to-PKL preprocessing while preserving the approved Option 1
train/val/test split and never scanning the full TUAB dataset.
"""
...
DEFAULT_OUTPUT_ROOT = ROOT / "data" / "biot_option1_original_processed"
REPORT_JSON = ROOT / "reports" / "biot_option1_preprocessing_wrapper_summary.json"
REPORT_MD = ROOT / "reports" / "biot_option1_preprocessing_wrapper_report.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="BIOT Option 1 TUAB preprocessing wrapper.")
...
def subset_edf_path(raw_subset_root: Path, row: dict[str, Any]) -> Path:
    return raw_subset_root / row["canonical_split"] / LABEL_NAMES[int(row["label"])] / row["edf_basename"]
...
def channel_index(ch_names: list[str], wanted: str) -> int:
    ...
    upper = {name.upper(): idx for idx, name in enumerate(ch_names)}
    key = wanted.upper()
    if key in upper:
        return upper[key]
...
def process_one_edf(edf_path: Path, output_dir: Path, label: int) -> dict[str, Any]:
    ...
    for i in range(n_windows):
        out_path = output_dir / f"{edf_path.stem}_{i}.pkl"
...
def write_reports(summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# BIOT Option 1 Preprocessing Wrapper Report",
        "",
        ...
        "",
        "## Original BIOT TUAB Transform",
        "",
        "- Resample to 200 Hz.",
        "- No bandpass filter is applied in BIOT `datasets/TUAB/process.py`.",
        "- No notch filter is applied in BIOT `datasets/TUAB/process.py`.",
        "- Build 16 bipolar channels from raw EDF signals.",
