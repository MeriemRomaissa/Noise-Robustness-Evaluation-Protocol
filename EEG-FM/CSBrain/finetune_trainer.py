...
def _add_benchmark_finetuning_path_to_syspath():
    """Let CSBrain reuse Benchmark LoRA helpers without changing model code."""
...
def _apply_benchmark_finetune_strategy(model, params):
    ...
    params.model_name = "CSBrain"
    params.allow_head_guess = True
    summary = lora_csbrain.apply_strategy(model, params)
        ...
        if self.params.optimizer == 'AdamW':
            ...
            if benchmark_strategy_active:
                ...
                if not trainable_params:
                    raise RuntimeError("No trainable parameters found for CSBrain optimizer.")
                self.optimizer = torch.optim.AdamW(
                    trainable_params,
                    lr=self.params.lr,
                    weight_decay=self.params.weight_decay,
                )
            elif self.params.multi_lr: # set different learning rates for different modules
                self.optimizer = torch.optim.AdamW([
