...
def _first_seed(value):
    """Original CBraMod trains one seed per process; YAML may list many."""
    if isinstance(value, (list, tuple)):
        return int(value[0]) if value else 3407
    return int(value)
...
def _config_defaults(config):
    """Translate Benchmark YAML fields to native CBraMod argparse defaults."""
    defaults = {}
    ...
    for key in ("downstream_dataset", "num_of_classes", "classifier", "dropout",
...
def load_unified60_tuab_dataset(params):
    """Read unified60 H5 rows through the Benchmark CBraMod loader."""
    _add_benchmark_paths_to_syspath()
    from loader_cbramod import build_unified60_loader
...
def _require_dataset(module, dataset_name):
    ...
    if module is None:
        raise ImportError(
            f"CBraMod original dataset module for {dataset_name} is unavailable. "
...
def main():
    ...
    parser.add_argument('--multi_lr', type=_str_to_bool, default=True,
    ...
    parser.add_argument('--data_source', type=str, default='original',
                        ...
                        help='original uses CBraMod processed data; tuab_unified60 uses Benchmark H5 loaders.')
    #newly added codes
